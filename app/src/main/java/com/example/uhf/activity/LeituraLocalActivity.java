...
public class LeituraLocalActivity extends AppCompatActivity {
    ...
    // 🔥 FIX: executor único single-thread garante ordem de execução
    ...
    private volatile boolean regraCarregada = false;
    ...
    private int  ignorarEsquerda = 0;
    private int  ignorarDireita  = 0;
    ...
    public static class TagItem {
        ...
        public boolean statusConfirmado = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (localJson != null) {
            try { localizacaoSelecionada = new JSONObject(localJson); }
            catch (Exception e) { Log.e(TAG, "Erro parse localização", e); }
        }

        // Hardware inicializa em paralelo (não depende da regra)
        ...
        if (localizacaoSelecionada != null) {
            ...
            // 🔥 FIX: single-thread garante esta ordem:
            // 1º propriedades (carrega ignorarEsquerda da API)
            // 2º banco        (já usa a regra correta)
            // 3º API          (já usa a regra correta)
            rfidExecutor.execute(this::carregarPropriedadesSistema);
            rfidExecutor.execute(() -> carregarAtivosDoBancoSync(localId));
            rfidExecutor.execute(this::carregarCamposEAtivosSync);
        }
        ...
        rbLoop.setOnCheckedChangeListener((b, checked) -> { if (checked) modoSingle = false; });
        rbSingle.setOnCheckedChangeListener((b, checked) -> { if (checked) modoSingle = true; });
            ...
            for (TagItem tag : listaTags) {
                if ("ENCONTRADO".equals(tag.status) || "MOVIMENTADO".equals(tag.status)) {
                    try {
                        ...
                        tagsLidas.put(obj);
                    } catch (Exception e) { Log.e(TAG, "Erro montar tag relatorio", e); }
                }
            }
    ...
    private void trocarModo(boolean paraRfid) {
        modoRfid = paraRfid;
        modo2D   = !paraRfid;
        if (modoRfid) pararLeitura2D(); else pararLeituraRFID();
        atualizarEstadoBotoes();
    }

    private void alternarLeituraPrincipal() {
        if (modoRfid) { if (isReadingRFID) pararLeituraRFID(); else iniciarLeituraRFID(); }
        else          { if (isReading2D)   pararLeitura2D();   else iniciarLeitura2D();   }
    }
    ...
    private void inicializarBarcode2D() {
        ...
        } catch (Exception e) { Log.e(TAG, "Erro inicializando 2D", e); }
    }
    ...
    private void configurarCallback2D() {
        barcodeDecoder.setDecodeCallback(barcodeEntity -> {
            ...
            // 🔥 FIX: aguarda regra antes de normalizar
            aguardarRegraCarregada();
    ...
    private void iniciarLeitura2D() {
        ...
        catch (Exception e) { Log.e(TAG, "Erro iniciando 2D", e); isReading2D = false; }
    }

    private void pararLeitura2D() {
        if (barcodeDecoder == null) return;
        isReading2D = false;
        try { barcodeDecoder.stopScan(); }
        catch (Exception e) { Log.e(TAG, "Erro parando 2D", e); }
    }
    ...
    private void carregarPropriedadesSistema() {
        try {
            ...
            String response = new Scanner(new java.net.URL(baseUrl + "mobile/propriedades_do_sistema")
                    .openStream()).useDelimiter("\\A").next();

            // 🔥 A API retorna um JSONObject direto, não um array
            JSONObject obj = new JSONObject(response);

            ignorarEsquerda = obj.optInt("tagIgnorarCaracterEsquerda", 0);
            ignorarDireita  = obj.optInt("tagIgnorarCaracterDireita",  0);

            Log.d(TAG, "🔧 Regra carregada: Esq=" + ignorarEsquerda + " Dir=" + ignorarDireita);

        } catch (Exception e) {
            Log.e(TAG, "Erro carregar propriedades sistema", e);
        } finally {
            regraCarregada = true; // 🔥 libera sempre
        }
    }

    private void aguardarRegraCarregada() {
        int tentativas = 0;
        while (!regraCarregada && tentativas++ < 30) {
            try { Thread.sleep(100); } catch (InterruptedException ignored) {}
        }
        if (!regraCarregada)
            Log.w(TAG, "⚠️ Regra não carregada após 3s — usando padrão 0");
    }

    /**
     * 🔥 FIX: bloqueia até a regra estar disponível. Máx 3 segundos.
     * Usado apenas no callback do barcode (roda fora do rfidExecutor).
     */
    private String aplicarRegraEpc(String epc) {
        if (epc == null) return "";
        epc = epc.trim();

        Log.d(TAG, "🔍 aplicarRegraEpc: epc=" + epc + " ignorarEsquerda=" + ignorarEsquerda + " regraCarregada=" + regraCarregada);

        // 🔥 Se o EPC já é menor ou igual ao que sobraria após o corte,
        // está normalizado — não aplica a regra de novo
        if (epc.length() <= ignorarEsquerda) {
            Log.d(TAG, "🔍 EPC já normalizado, retorna como está: " + epc);
            return epc;
        }

        // 1️⃣ Remove N caracteres da ESQUERDA
        epc = epc.substring(ignorarEsquerda);

        if (epc.isEmpty()) return "0";

        // 2️⃣ Rtrim de zeros à DIREITA — só se tiver pelo menos um dígito não-zero
        boolean temNaoZero = false;
        for (char c : epc.toCharArray()) {
            if (c != '0') { temNaoZero = true; break; }
        }
        if (temNaoZero) {
        ...
        Log.d(TAG, "🔍 resultado após regra: " + epc);
        return epc;
    }

    // ===========================
    // PASSO 1 — BANCO
    // (roda APÓS carregarPropriedadesSistema — garantido pelo executor)
    // ===========================
    private void carregarAtivosDoBancoSync(int localIdSelecionado) {
        try {
            ...
            for (JSONObject ativoDB : ativosNoLocal) {
                Log.d(TAG, "📦 rfid do banco: " + ativoDB.optString("rfid", ""));
                String epcCompleto = ativoDB.optString("rfid", "").trim();
                if (epcCompleto.isEmpty()) continue;
                String chave = epcCompleto;
            ...
            for (TagItem tag : listaTags) {
                if (!mapaLocais.containsKey(tag.localizacaoId)) {
                    mapaLocais.put(tag.localizacaoId, dbHelper.buscarNomeLocalPorId(tag.localizacaoId));
                }
            }
        ...
        } catch (Exception e) { Log.e(TAG, "Erro carregar banco", e); }
    }
                        ...
                        try {
                            if (mReader != null && mReader.setPower(power))
                                mainHandler.post(() -> Toast.makeText(this,
                                        "Potência ajustada: " + power + " dBm", Toast.LENGTH_SHORT).show());
                        } catch (Exception e) {
                            mainHandler.post(() -> Toast.makeText(this,
                                    "Erro ao ajustar potência: " + e.getMessage(), Toast.LENGTH_SHORT).show());
                        }
                    });
                }).show();
    }
    ...
    // ===========================
    // PASSO 2 — API
    // (roda APÓS carregarAtivosDoBancoSync — garantido pelo executor)
    // ===========================
    private void carregarCamposEAtivosSync() {
        try {
            ...
            for (int i = 0; i < localizacoesApi.length(); i++) {
                ...
                if (idLoc != -1) { mapaLocais.put(idLoc, nome); dbHelper.salvarOuAtualizarLocalizacao(loc, "LOCAL"); }
            }
            ...
            mapaCampos.clear(); mapaNiveis.clear();
            for (int i = 0; i < arrCampos.length(); i++) { JSONObject c = arrCampos.getJSONObject(i); mapaCampos.put(c.getInt("id"), c); }
            for (int i = 0; i < arrNiveis.length(); i++) { JSONObject n = arrNiveis.getJSONObject(i); mapaNiveis.put(n.getInt("id"), n); }
            ...
            for (int i = 0; i < ativosApi.length(); i++) {
                ...
                String epc = ativo.optString("rfid", "").trim();
                if (!epc.isEmpty()) mapaTodosAtivos.put(epcChave(epc), ativo);
            }
            ...
            for (Map.Entry<String, JSONObject> entry : mapaTodosAtivos.entrySet()) {
                ...
                String     descricao    = (listaMulti != null && listaMulti.length() > 0)
                        ? descobrirDescricao(listaMulti) : ativo.optString("descricao", "Sem descrição");

                JSONObject ativoDB = new JSONObject();
                ativoDB.put("id", ativo.optInt("id")); ativoDB.put("rfid", epcCompleto);
                ativoDB.put("descricao", descricao);   ativoDB.put("status", "NO LOCAL");
                ...
                if (localIdAtivo == localIdSelecionado) {
                    if (mapaTags.containsKey(chave)) {
                        ...
                        tag.epcChave = chave; tag.descricao = descricao; tag.status = "NO LOCAL";
                        listaTags.add(tag); mapaTags.put(chave, tag);
            ...
            for (TagItem tag : listaTags) {
                if ("NO LOCAL".equals(tag.status) && !mapaTodosAtivos.containsKey(tag.epcChave)) {
                    ...
                    Log.d(TAG, "Não existe na API → NAO_ENCONTRADO: " + tag.epcChave);
                }
            }

            Log.d(TAG, "✅ Após sync API — lista total: " + listaTags.size());
            mainHandler.post(() -> {
                adapter.notifyDataSetChanged(); atualizarContador();
                tvQtdLocal.setText("Total de itens: " + listaTags.size()); ajustarAlturaListView();
            });
        } catch (Exception e) { Log.e(TAG, "Erro carregar API", e); }
    }

    private void mostrarOpcoesStatus(TagItem tag) {
        String[] opcoes;
        switch (tag.status) {
            case "NO LOCAL":       opcoes = new String[]{"✅ ENCONTRADO", "\uD83D\uDD04 MOVIMENTADO", "❌ NÃO ENCONTRADO"}; break;
            case "NAO_ENCONTRADO": opcoes = new String[]{"✅ ENCONTRADO", "\uD83D\uDD04 MOVIMENTADO"}; break;
            case "MOVIMENTADO":    opcoes = new String[]{"🔄 Confirmar MOVIMENTADO", "❌ NÃO ENCONTRADO", "✅ ENCONTRADO"}; break;
            case "ENCONTRADO":     opcoes = new String[]{"\uD83D\uDD04 MOVIMENTADO", "❌ NÃO ENCONTRADO"}; break;
            default:               opcoes = new String[]{"✅ ENCONTRADO", "\uD83D\uDD04 MOVIMENTADO", "❌ NÃO ENCONTRADO"}; break;
        }
        new AlertDialog.Builder(this)
                ...
                .setItems(opcoes, (dialog, which) -> {
                    ...
                    if (opcao.contains("MOVIMENTADO")) {
                        ...
                        tag.status = novoStatus; tag.statusConfirmado = true;
                        Log.d(TAG, "✏️ Reclassificado: " + tag.epcChave + " → " + novoStatus);
                        salvarStatusNoBanco(tag, novoStatus);
                        adapter.notifyDataSetChanged(); atualizarContador(); ajustarAlturaListView();
                    }
                })
    ...
    private void confirmarMovimentado(TagItem tag) {
        String nomeLocalOrigem  = mapaLocais.getOrDefault(tag.localizacaoId, "Local " + tag.localizacaoId);
        int    localDestinoId   = obterUltimaLocalizacao(localizacaoSelecionada);
        String nomeLocalDestino = mapaLocais.getOrDefault(localDestinoId, obterNomeLocal(localizacaoSelecionada));
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("Sim, confirmar", (dialog, which) -> {
                    tag.status = "MOVIMENTADO"; tag.statusConfirmado = true;
    ...
    private void salvarStatusNoBanco(TagItem tag, String status) {
        try {
            ...
            String statusReal = ("MOVIMENTADO".equals(status) && !tag.statusConfirmado) ? "NAO_ENCONTRADO" : status;
            salvar.put("status",        statusReal);
            salvar.put("localizacaoId", tag.localizacaoId);
            dbHelper.salvarOuAtualizarAtivoSimples(salvar);
        } catch (Exception e) { Log.e(TAG, "Erro salvar status no banco", e); }
    }
    ...
    private void inicializarRFID() {
        ...
        } catch (Exception e) { Log.e(TAG, "❌ Erro inicializar RFID", e); }
    }
    ...
    private void iniciarLeituraRFID() {
        ...
        isReadingRFID = true;
        rfidExecutor.execute(() -> {
            // 🔥 FIX: aguarda regra antes de processar qualquer tag
            aguardarRegraCarregada();
            try {
            ...
            } catch (Exception e) { Log.e(TAG, "Erro leitura RFID", e); }
        });
        if (modoSingle) mainHandler.postDelayed(this::pararLeituraRFID, 250);
    }
    ...
    private void pararLeituraRFID() {
        ...
        rfidExecutor.execute(() -> {
            ...
            catch (Exception e) { Log.e(TAG, "Erro parar RFID", e); }

            for (TagItem tag : listaTags) {
                if ("NO LOCAL".equals(tag.status)) {
                    tag.status = "NAO_ENCONTRADO";
                    Log.d(TAG, "Não lido → NAO_ENCONTRADO: " + tag.epcChave);
                    try {
                        JSONObject salvar   = new JSONObject();
                        JSONObject ativoApi = mapaTodosAtivos.get(tag.epcChave);
                        if (ativoApi != null) salvar.put("id", ativoApi.optInt("id"));
                        ...
                        salvar.put("localizacaoId", tag.localizacaoId);
                        dbHelper.salvarOuAtualizarAtivoSimples(salvar);
                    } catch (Exception e) { Log.e(TAG, "Erro salvar NAO_ENCONTRADO", e); }
                }
            }
            mainHandler.post(() -> { adapter.notifyDataSetChanged(); atualizarContador(); ajustarAlturaListView(); });
        });
    }
    ...
    private synchronized void processarTagLida(String epcLida) {
        ...
        String status, descricao; int localBanco;
        ...
        if (ativoApi != null) {
            ...
            descricao    = (listaMulti != null && listaMulti.length() > 0)
                    ? descobrirDescricao(listaMulti) : ativoApi.optString("descricao", "Sem descrição");
            status       = (localBanco == localSelecionado) ? "ENCONTRADO" : "MOVIMENTADO";
        } else {
            descricao = "TAG NÃO CADASTRADA"; status = "NAO_ENCONTRADO"; localBanco = localSelecionado;
        }

        TagItem tagExistente = mapaTags.get(chave);
        if (tagExistente != null) {
            if (!tagExistente.statusConfirmado) tagExistente.status = status;
            tagExistente.descricao = descricao; tagExistente.localizacaoId = localBanco;
            Log.d(TAG, "✅ ATUALIZADO: " + chave + " → " + tagExistente.status);
        } else {
            TagItem nova = new TagItem(chave, localBanco);
            nova.epcChave = chave; nova.descricao = descricao; nova.status = status;
            listaTags.add(nova); mapaTags.put(chave, nova);
        ...
        try {
            ...
            salvar.put("status",        "MOVIMENTADO".equals(status) ? "NAO_ENCONTRADO" : status);
            salvar.put("localizacaoId", localBanco);
            dbHelper.salvarOuAtualizarAtivoSimples(salvar);
        } catch (Exception e) { Log.e(TAG, "Erro salvar no banco", e); }

        tagsPendentes++;
        long agora = System.currentTimeMillis();
        if (agora - ultimoUpdateUI >= 300) {
            ultimoUpdateUI = agora; tagsPendentes = 0;
            mainHandler.post(() -> { adapter.notifyDataSetChanged(); atualizarContador(); ajustarAlturaListView(); });
        }
    }
    ...
    private void limparTags() {
        pararLeituraRFID();
        listaTags.clear(); mapaTags.clear();
        adapter.notifyDataSetChanged(); atualizarContador(); ajustarAlturaListView();
    ...
    private void ajustarAlturaListView() {
        if (adapter == null || adapter.getCount() == 0) {
            listViewTags.getLayoutParams().height = 1; listViewTags.requestLayout(); return;
        }
    ...
    private void concluirInventario() {
        ...
        for (TagItem tag : listaTags) {
            if ("ENCONTRADO".equals(tag.status) || "MOVIMENTADO".equals(tag.status) || "NAO_ENCONTRADO".equals(tag.status)) {
                try {
                    String statusParaApi;
                    if ("MOVIMENTADO".equals(tag.status)) {
                        if (tag.statusConfirmado) { statusParaApi = "MOVIMENTADO"; }
                        else { statusParaApi = "NAO_ENCONTRADO"; tag.status = "NAO_ENCONTRADO";
                            Log.d(TAG, "🔒 PROTEGIDO: " + tag.epcChave + " → NAO_ENCONTRADO"); }
                    } else { statusParaApi = tag.status; }
                    JSONObject obj = new JSONObject();
                    obj.put("epc", tag.epc); obj.put("status", tag.status); obj.put("statusApi", statusParaApi);
                    tagsArray.put(obj);
                } catch (Exception e) { Log.e(TAG, "Erro montar tag para envio", e); }
            }
        }
    ...
    // AUXILIARES
    // ===========================
    private String epcChave(String epc)    { return normalizarEpc(epc); }
    private String normalizarEpc(String epc) { return aplicarRegraEpc(epc); }

    private JSONArray lerUrl(String url) throws Exception {
        return new JSONArray(new Scanner(new java.net.URL(url).openStream()).useDelimiter("\\A").next());
    }

    private void salvarLocalizacaoNoBanco(JSONObject local) {
        try { dbHelper.salvarOuAtualizarLocalizacao(local, "LOCAL"); }
        catch (Exception e) { Log.e(TAG, "Erro salvar local no DB", e); }
    }
    ...
    private String obterNomeLocal(JSONObject local) {
        ...
        } catch (Exception e) { Log.e(TAG, "Erro nome local", e); }
        return "Desconhecido";
    }
    ...
    private int obterUltimaLocalizacao(JSONObject local) {
        ...
        } catch (Exception e) { Log.e(TAG, "Erro obter localização", e); }
        return -1;
    }
    ...
    private String descobrirDescricao(JSONArray listaIds) {
        ...
        } catch (Exception e) { Log.e(TAG, "Erro descobrir descrição", e); }
